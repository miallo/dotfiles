...
# markQueue: put i3 marks using the window ids stored on the queue.
# The windows are marked using "_prevFocusX" where X is a integer corresponding with the index of the element on the queue.
# The previously focused window (the most recent) is marked using "_prevFocus0".
# This function also cleans the queue from windows that can't be marked (e.g. windows that have been closed).
markQueue(){
    queueCopy=( "${queue[@]}" )
    queue=( )
    j=0
